...
void dce_global_variables_setup (struct DceGlobalVariables *variables)
{
  ...
  *process->pstdin = fdopen (0, "r");
  *process->pstdout = fdopen (1, "a"); // Append in order to not erase content while doing an exec
  *process->pstderr = fdopen (2, "a"); // Append in order to not erase content while doing an exec
